module SpecMaker
	...
	MD_API_STRUCTURE = "../jsonFiles/template/api_template.md"
	MD_RESOURCE_STRUCTURE = "../jsonFiles/template/resource_template.md"

	@at = File.readlines(MD_API_STRUCTURE, :encoding => 'UTF-8')
	@rt = File.readlines(MD_RESOURCE_STRUCTURE, :encoding => 'UTF-8')
	...
	def self.get_property_desc(p=nil, isRelationship=false)
		desc = p[:description]
		(desc = desc + TIMESTAMP_DESC) if p[:dataType].downcase == 'datetimeoffset'					
		if p[:enumName] && (@enumHash.has_key? p[:enumName])
			appendEnum = " Possible values are: `" + @enumHash[p[:enumName]]["options"].keys.join('`, `') + "`."
			desc = desc + appendEnum
		end
		
		if !isRelationship
			desc = (p[:isReadOnly] || p[:isKey]) ? desc  + ' Read-only.' : desc
			desc = p[:isNullable] ? desc + ' Nullable.' : desc
		end
		return desc
	end

	# If the type is of	an object, then provide markdown link.
	def self.get_datatype_pluslink(p=nil)
		dataTypePlusLink = p[:dataType] 	
		if !(SIMPLETYPES.include? p[:dataType])			
			dataTypePlusLink = "[" + p[:dataType] + "](" + p[:dataType].downcase + ".md)"
		end				
		(dataTypePlusLink = dataTypePlusLink + " collection") if p[:isCollection] 
		return dataTypePlusLink				
	end
